import React from 'react';
import {Helmet} from "react-helmet";

import BulkDeleteForm from './BulkDeleteForm.jsx';

// CSS
import '../../../../css/Support.css';

const BulkDelete = (props) => {
  if(process.env.REACT_APP_ENV === "production") {
    window.Intercom("update");
  }

  return (
    <div styleName="container">
        <Helmet>
          <title>Delete Multiple Accounts</title>
          <meta name="description" content="Want to delete your data in bulk? We'd love to hear from you"/>
        </Helmet>
      <div styleName="left">
        <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="52" height="52" viewBox="0 0 52 52">
          <image id="Vector_Smart_Object" data-name="Vector Smart Object" width="52" height="52" xlinkHref="data:img/png;base64,"/>
        </svg>
        <div>
          <h2>Steps</h2>
          <br/>
          <p>
            <span>1. Fill in your request 👉</span>
            <br/><br/>
            <span>2. We'll send you an email acknowledging acceptance of your request.</span>
            <br/><br/>
            <span>3. Within 24hrs, you'll receive quite simply the easiest way for you to delete your data.</span>
          </p>
        </div>
      </div>        
      <div styleName="right">
        <h1>Delete Multiple Accounts</h1>
        <BulkDeleteForm/>
      </div>    
    </div>
  )
}

export default BulkDelete